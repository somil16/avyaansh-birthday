...
def create_birthday_video():
    ...
    def add_balloons_animation(clip):
        """Add floating balloons animation"""
        # This is a simplified version - you can enhance with actual balloon images
        balloon_colors = [(255, 0, 0), (0, 255, 0), (0, 0, 255), (255, 255, 0)]
        balloons = []
        
        for i, color in enumerate(balloon_colors):
            x_pos = 200 + i * 400
            balloon = ColorClip(size=(50, 80), color=color, duration=clip.duration)
            balloon = balloon.set_position((x_pos, lambda t: HEIGHT - 100 - 50 * np.sin(t + i)))
            balloons.append(balloon)
        
        return CompositeVideoClip([clip] + balloons)
    ...
    slide1 = add_balloons_animation(slide1)
    clips.append(slide1)
...
as we celebrate Avyaansh turning ONE!"""
    ...
    slide6 = add_balloons_animation(slide6)
    clips.append(slide6)
    
    # Concatenate all clips
    final_video = concatenate_videoclips(clips, method="compose")
    
    # Add background music (you'll need to provide a music file)
    # Uncomment and modify the path to your music file:
    # audio = AudioFileClip("path/to/your/birthday_music.mp3").subclip(0, final_video.duration)
...
def create_video_with_photos():
    ...
    photo_paths = [
        "0M3A3188.JPG",  # Update these with actual paths
        "0M3A3228.JPG",
        "0M3A3313.JPG",
        "0M3A3316.JPG"
    ]
...
# INSTALLATION INSTRUCTIONS (Run in Google Colab or local Python environment):
"""
1. Install required libraries:
   !pip install moviepy pillow numpy

2. For Google Colab, upload this script and your photos

3. Run the script:
   python create_birthday_video.py

4. The video will be saved as 'avyaansh_birthday_invitation.mp4'

CUSTOMIZATION OPTIONS:
- Change colors by modifying the RGB values in create_background_clip()
- Adjust text size with the fontsize parameter
- Modify slide duration with DURATION_PER_SLIDE
- Add your own background music by uncommenting the audio section
- Include actual photos by updating the photo_paths list
